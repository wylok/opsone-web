    ...
    <div class="app-div">
      <el-container>
        <el-header height="50px">
          <div style="border: 1px solid #a7b6cd;height: 45px;border-radius: 5px;background-color: steelblue">
            <div style="margin-top: 5px">
              <el-row :gutter="24">
                ...
                <el-col :span="2">
                  <el-button type="warning" plain size="small" @click="statefulSetRestart">重启statefulSet</el-button>
                </el-col>
                <el-col :span="2">
                  <el-button type="danger" plain size="small" @click="delStatefulSet">删除statefulSet</el-button>
                </el-col>
              </el-row>
            </div>
          </div>
        ...
        <el-container style="padding: 0 15px;height: 700px;">
            ...
            >
              ...
              <div style="margin-top: 15px;font-size: 12px">
                <el-row :gutter="24">
                  ...
                  <el-col :span="7">
                    <span class="flag">H</span>{{ v['HostIP'] }}
                  </el-col>
                  <el-col :span="7">
          ...
          <el-aside width="65%" style="padding-left: 8px;margin-top: 5px">
            <div style="font-size: 13px;border: 2px solid #7b91b5;border-radius: 8px;height: 200px;width: 98%;margin-bottom: 15px">
              <div class="pod-detail">容器基础信息</div>
              <div style="margin-top: 15px;text-overflow: ellipsis;overflow: hidden;">
                <el-descriptions v-if="podInfos!==null" border :labelStyle="{'font-weight': 'bold'}">
                  <el-descriptions-item label="PodName">
                    <el-col class="col-hide">{{ podInfos['Name'] }}</el-col>
                  </el-descriptions-item>
                  <el-descriptions-item label="PodIP">{{ podInfos['PodIP'] }}</el-descriptions-item>
                  <el-descriptions-item label="NodeName">{{ podInfos['NodeName'] }}</el-descriptions-item>
                  <el-descriptions-item label="HostIP">{{ podInfos['HostIP'] }}</el-descriptions-item>
                  ...
                  <el-descriptions-item v-for="(v,k) in podInfos['images']" :key="k" label="容器镜像">
                ...
                <el-row :gutter="24">
                  <el-col :span="10" :offset="2">
                    <div style="margin-top: 35px">
                      CPU资源请求:
                      <el-input v-model="cpu1" style="width: 55%" size="mini" readonly>
                        <template slot="append">core</template>
                      </el-input>
                    </div>
                    <div style="margin-top: 35px">
                      CPU资源限制:
                      <el-input v-model="cpu2" style="width: 55%" size="mini" readonly>
                        <template slot="append">core</template>
                      </el-input>
                    </div>
                  </el-col>
            ...
            <div style="font-size: 13px;border: 2px solid #7b91b5;border-radius: 8px;height: 430px;width: 98%;">
              <div class="pod-detail">容器关联事件</div>
              <el-table
                v-if="podEvents !== null"
                :data="podEvents"
                border
              ...
              <el-empty v-else :image-size="130" description="没有该 Pod 的关联事件" />
            </div>
          </el-aside>
        </el-container>
      </el-container>
...
export default {
  ...
  created() {
    ...
    this.statefulSet = this.$route.params['name']
    this.content = this.k8s_name + '  |  ' + this.namespace + '  |  ' + this.statefulSet
    this.getData()
  ...
  methods: {
    ...
    getData() {
      ...
      this.scaler = null
      getK8sPods({ k8s_id: this.k8s_id, namespace: this.namespace, statefulSet: this.statefulSet }).then((res) => {
        if (res.success) {
          this.list = res.data
      ...
      getStatefulSetYaml({ k8s_id: this.k8s_id, namespace: this.namespace, name: this.statefulSet }).then((res) => {
        if (res.success) {
          this.textarea = res.data
        }
      })
      getAutoScalers({ k8s_id: this.k8s_id, namespace: this.namespace, statefulSet: this.statefulSet }).then((res) => {
        if (res.success) {
          this.scalerName = this.statefulSet
    ...
    delStatefulSet() {
      this.$confirm('此操作将移除' + this.statefulSet + '并销毁所属pod,是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteStatefulSet({ k8s_id: this.k8s_id, namespace: this.namespace, name: this.statefulSet }).then((res) => {
          if (res.success) {
            this.$message({
              type: 'success',
              message: this.statefulSet + '已移除'
            })
          }
        }).catch(function(err) {
          if (err.toString().includes('403')) {
    ...
    statefulSetRestart() {
      this.$confirm('此操作将重启' + this.statefulSet + '是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        restartStatefulSet({ k8s_id: this.k8s_id, namespace: this.namespace, name: this.statefulSet }).then((res) => {
          if (res.success) {
            this.$message({
              type: 'success',
              message: this.statefulSet + '已重启'
            })
          }
        }).catch(function(err) {
          if (err.toString().includes('403')) {
    ...
    modifyImages(images, msg) {
      updateDaemonSet({ k8s_id: this.k8s_id, namespace: this.namespace, name: this.statefulSet, images: images }).then((res) => {
        if (res.success) {
          this.$message({
            type: 'success',
            message: msg
