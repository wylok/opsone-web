          ...
          <div style="border: 1px solid #a7b6cd;height: 45px;border-radius: 5px;background-color: steelblue">
            <div style="margin-top: 5px">
              <el-row :gutter="24">
                ...
                <el-col :span="2">
                  <el-button type="warning" plain size="small" @click="DaemonSetRestart">重启DaemonSet</el-button>
                </el-col>
                <el-col :span="2">
                  <el-button type="danger" plain size="small" @click="delDaemonSet">删除DaemonSet</el-button>
                </el-col>
              </el-row>
            </div>
          </div>
            ...
            <div style="font-size: 13px;border: 2px solid #7b91b5;border-radius: 8px;height: 430px;width: 98%;">
              ...
              <div v-else>
                <div v-if="podMemoryChart.tagData.length>0">
                  <LineChart :chart-data="podCpuChart" height="200px" />
                  <LineChart :chart-data="podMemoryChart" height="200px" style="margin-top: 5px" />
                </div>
...
export default {
  ...
  created() {
    ...
    this.daemonSet = this.$route.params['name']
    this.content = this.k8s_name + '  |  ' + this.namespace + '  |  ' + this.daemonSet
    this.getData()
  ...
  methods: {
    ...
    getData() {
      ...
      this.scaler = null
      getK8sPods({ k8s_id: this.k8s_id, namespace: this.namespace, daemonSet: this.daemonSet }).then((res) => {
        if (res.success) {
          this.list = res.data
      ...
      getDaemonSetYaml({ k8s_id: this.k8s_id, namespace: this.namespace, name: this.daemonSet }).then((res) => {
        if (res.success) {
          this.textarea = res.data
        }
      })
      getAutoScalers({ k8s_id: this.k8s_id, namespace: this.namespace, daemonSet: this.daemonSet }).then((res) => {
        if (res.success) {
          this.scalerName = this.daemonSet
    ...
    delDaemonSet() {
      this.$confirm('此操作将移除' + this.daemonSet + '并销毁所属pod,是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        deleteDaemonSet({ k8s_id: this.k8s_id, namespace: this.namespace, name: this.daemonSet }).then((res) => {
          if (res.success) {
            this.$message({
              type: 'success',
              message: this.daemonSet + '已移除'
            })
          }
        }).catch(function(err) {
          if (err.toString().includes('403')) {
    ...
    DaemonSetRestart() {
      this.$confirm('此操作将重启' + this.daemonSet + '是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        restartDaemonSet({ k8s_id: this.k8s_id, namespace: this.namespace, name: this.daemonSet }).then((res) => {
          if (res.success) {
            this.$message({
              type: 'success',
              message: this.daemonSet + '已重启'
            })
          }
        }).catch(function(err) {
          if (err.toString().includes('403')) {
    ...
    modifyImages(images, msg) {
      updateDaemonSet({ k8s_id: this.k8s_id, namespace: this.namespace, name: this.daemonSet, images: images }).then((res) => {
        if (res.success) {
          this.$message({
            type: 'success',
            message: msg
