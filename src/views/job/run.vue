    ...
    <div class="app-div">
      ...
      >
        <el-table-column
          ...
          width="55"
        />
        <el-table-column label="作业ID" align="center" show-overflow-tooltip width="250px">
        ...
        <el-table-column label="执行脚本" width="auto" align="center">
          <template slot-scope="scope">
            <router-link
              :to="{ name:'job_script_search', params: { script_ids: scope.row['script_id'] } }"
            >
              <el-link type="primary">{{ scope.row['script_name'] }}</el-link>
            </router-link>
          </template>
        </el-table-column>
        <el-table-column label="定时执行" align="center" width="auto">
          <template slot-scope="scope">
            <span>{{ scope.row['cron']|jobFilter }}</span>
          </template>
        </el-table-column>
        <el-table-column align="center" prop="run_time" label="执行时间" width="180px">
          <template slot-scope="scope">
            <i class="el-icon-time" />
            {{ scope.row['run_time'] | DateFormat }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="作业结果" align="center" width="auto">
          <template slot-scope="scope">
            <el-link
              ...
              type="primary"
              @click="getJobDetail(scope.row['job_id'],scope.row['host_id'],scope.row['script_id'])"
            >查看
            </el-link>
      ...
      >
        <template>
          <div style="margin-left: 3%;margin-right: 3%;background:black;color:white;width:95%">
...
import { getJobResults, getJobScript, getJobScriptRun } from '@/api/job'
...
export default {
  ...
  methods: {
    ...
    async getData(params) {
      ...
      let data = []
      await getJobScriptRun(params).then((res) => {
        if (res.success) {
          data = res.data
          this.hostIds = []
          this.scriptIds = []
          res.data.forEach((v) => {
            ...
            if (this.scriptIds.indexOf(v.script_id) === -1) {
              this.scriptIds.push(v.script_id)
            }
          })
      ...
      await getJobScript({ script_ids: this.scriptIds.join(',') }).then((res) => {
        if (res.success) {
          this.scriptNames = []
          res.data.forEach((v) => {
            this.scriptNames[v.script_id] = v.script_name
          })
        }
      })
      for (const v of data) {
        v['host_name'] = this.hostNames[v.host_id]
        v['script_name'] = this.scriptNames[v.script_id]
      }
      this.list = data
      this.listLoading = false
    },
    getJobDetail(jobId, hostId, scriptId) {
      getJobResults({ job_id: jobId, host_id: hostId }).then((res) => {
        if (res.success) {
          this.jobResults = []
          this.DrawerTitle = '作业ID:' + jobId + ' 主机:' + this.hostNames[hostId]
          this.jobResults.push('执行脚本(id:' + scriptId + ')结果:')
